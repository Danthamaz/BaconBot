...
function addTodMob(name, lockoutHours, addedBy, pvp = false) {
  return getDb().prepare(`
    INSERT INTO tod_mobs (name, lockout_hours, pvp, added_by, added_at)
    VALUES (?, ?, ?, ?, ?)
  `).run(name, lockoutHours, pvp ? 1 : 0, addedBy || null, Date.now());
}
