...
function initSchema(db) {
  db.exec(`
    ...
    -- Guild bank: ledger of deposits/withdrawals (source of truth).
    -- Current inventory is always computed from this table, never stored.
    CREATE TABLE IF NOT EXISTS bank_transactions (
      id          INTEGER PRIMARY KEY AUTOINCREMENT,
      item_name   TEXT    NOT NULL COLLATE NOCASE,
      quantity    INTEGER NOT NULL,
      type        TEXT    NOT NULL CHECK (type IN ('deposit', 'withdraw')),
      mule        TEXT    NOT NULL COLLATE NOCASE,
      player_name TEXT,
      note        TEXT,
      recorded_by TEXT,
      recorded_at INTEGER NOT NULL
    ...
    CREATE INDEX IF NOT EXISTS idx_bank_tx_item ON bank_transactions(item_name COLLATE NOCASE, mule COLLATE NOCASE);
    CREATE INDEX IF NOT EXISTS idx_bank_tx_time ON bank_transactions(recorded_at DESC);

    -- Guild bank: persistent per-(item, mule) notes
    CREATE TABLE IF NOT EXISTS bank_item_notes (
      item_name  TEXT NOT NULL COLLATE NOCASE,
      mule       TEXT NOT NULL COLLATE NOCASE,
      note       TEXT NOT NULL,
      updated_by TEXT,
      updated_at INTEGER NOT NULL,
      PRIMARY KEY (item_name, mule)
    );
  `);

  // Migration: add pvp column to existing tod_mobs tables
  const cols = db.prepare("PRAGMA table_info(tod_mobs)").all();
  if (!cols.some(c => c.name === 'pvp')) {
    db.prepare("ALTER TABLE tod_mobs ADD COLUMN pvp INTEGER NOT NULL DEFAULT 0").run();
  }

  // Migration: add expansion column to existing tod_mobs tables
  if (!cols.some(c => c.name === 'expansion')) {
    db.prepare("ALTER TABLE tod_mobs ADD COLUMN expansion TEXT DEFAULT NULL").run();
  }
}
...
function saveRaid({ name, zone, startTime, endTime, characterName, submittedBy, attendance, loot }) {
  ...
  const stmtAttend = db.prepare(`
    INSERT OR REPLACE INTO attendance
      (raid_id, player_name, zone, level, class, race, guild, first_seen, last_seen)
    VALUES (@raidId, @playerName, @zone, @level, @class, @race, @guild, @firstSeen, @lastSeen)
  `);
...
/** All attendance rows for one raid, sorted by player name. Zones are aggregated per player. */
...
function getPlayerAttendance(term) {
  ...
  return getDb().prepare(`
    SELECT r.id, r.name, r.zone, r.start_time,
           a.player_name AS character_name, GROUP_CONCAT(DISTINCT a.zone) AS zones,
           MAX(a.level) AS level, MAX(a.class) AS class, MAX(a.guild) AS guild
    FROM attendance a
 ...
 * Uses zone = 'voice' as a sentinel so the UNIQUE(raid_id, player_name, zone)
 * constraint fires correctly (NULL != NULL in SQLite would cause duplicates).
 * On conflict, last_seen is updated only if the new value is later.
 */
function upsertVoiceAttendance(raidId, playerName, _discordId, firstSeen, lastSeen) {
  getDb().prepare(`
    INSERT INTO attendance (raid_id, player_name, zone, first_seen, last_seen)
    VALUES (@raidId, @playerName, 'voice', @firstSeen, @lastSeen)
    ON CONFLICT(raid_id, player_name, zone) DO UPDATE SET
      last_seen = CASE WHEN excluded.last_seen > attendance.last_seen THEN excluded.last_seen ELSE attendance.last_seen END
  `).run({
    raidId,
    playerName,
...
function addTodMob(name, lockoutHours, addedBy, pvp = false, expansion = null) {
  return getDb().prepare(`
    INSERT INTO tod_mobs (name, lockout_hours, pvp, added_by, added_at, expansion)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(name, lockoutHours, pvp ? 1 : 0, addedBy || null, Date.now(), expansion);
}

function updateTodMob(name, { lockoutHours, expansion } = {}) {
  const sets = [];
  const vals = [];
  if (lockoutHours != null) { sets.push('lockout_hours = ?'); vals.push(lockoutHours); }
  if (expansion !== undefined) { sets.push('expansion = ?'); vals.push(expansion); }
  if (sets.length === 0) return;
  vals.push(name);
  getDb().prepare(`UPDATE tod_mobs SET ${sets.join(', ')} WHERE name = ? COLLATE NOCASE`).run(...vals);
}
...
function getTodStatus(pvp = false) {
  return getDb().prepare(`
    ...
    WHERE m.pvp = ?
    ORDER BY m.expansion COLLATE NOCASE, m.name COLLATE NOCASE
  `).all(pvp ? 1 : 0);
}

// ── Guild bank operations ──────────────────────────────────────────────────

// Signed quantity: deposits add, withdrawals subtract.
const BANK_QTY_SUM = "SUM(CASE type WHEN 'deposit' THEN quantity ELSE -quantity END)";

/**
 * Record a bank deposit or withdrawal. Quantity is always positive.
 * Returns the new transaction id.
 */
function recordBankTransaction({ itemName, quantity, type, mule, playerName, note, recordedBy }) {
  const { lastInsertRowid } = getDb().prepare(`
    INSERT INTO bank_transactions (item_name, quantity, type, mule, player_name, note, recorded_by, recorded_at)
    VALUES (@itemName, @quantity, @type, @mule, @playerName, @note, @recordedBy, @recordedAt)
  `).run({
    itemName:   itemName.trim(),
    quantity,
    type,
    mule:       properCase(mule),
    playerName: playerName ? properCase(playerName) : null,
    note:       note || null,
...
/**
 * Current bank inventory computed from the ledger, one row per (item, mule)
 * with a positive quantity. Optional partial-item and exact-mule filters.
 */
function getBankInventory({ item, mule, limit = 15, offset = 0 } = {}) {
  const conds  = [];
  const params = [];
  if (item) { conds.push("t.item_name LIKE ? ESCAPE '\\'"); params.push(`%${item.replace(/[%_\\]/g, '\\$&')}%`); }
  if (mule) { conds.push('t.mule = ?');                     params.push(mule); }
  const where = conds.length ? `WHERE ${conds.join(' AND ')}` : '';
  return getDb().prepare(`
    SELECT t.item_name, t.mule, ${BANK_QTY_SUM} AS qty, n.note
    FROM bank_transactions t
    LEFT JOIN bank_item_notes n ON n.item_name = t.item_name AND n.mule = t.mule
    ${where}
    GROUP BY t.item_name, t.mule
    HAVING qty > 0
    ORDER BY t.item_name COLLATE NOCASE, t.mule COLLATE NOCASE
    LIMIT ? OFFSET ?
  `).all(...params, limit, offset);
}

/** Number of (item, mule) inventory rows, respecting the same filters as getBankInventory. */
function getBankInventoryCount({ item, mule } = {}) {
  const conds  = [];
  const params = [];
  if (item) { conds.push("item_name LIKE ? ESCAPE '\\'"); params.push(`%${item.replace(/[%_\\]/g, '\\$&')}%`); }
  if (mule) { conds.push('mule = ?');                     params.push(mule); }
  const where = conds.length ? `WHERE ${conds.join(' AND ')}` : '';
  return getDb().prepare(`
    SELECT COUNT(*) AS n FROM (
      SELECT 1 FROM bank_transactions
      ${where}
      GROUP BY item_name, mule
      HAVING ${BANK_QTY_SUM} > 0
    )
  `).get(...params).n;
}

/** Current quantity of one item on one mule (0 if none). Exact, case-insensitive match. */
function getBankItemQuantity(itemName, mule) {
  return getDb().prepare(`
    SELECT COALESCE(${BANK_QTY_SUM}, 0) AS qty
    FROM bank_transactions
    WHERE item_name = ? AND mule = ?
  `).get(itemName.trim(), mule).qty;
}

/** Mules currently holding an item, with quantities. Used for withdraw mule resolution. */
function getBankItemHoldings(itemName) {
  return getDb().prepare(`
    SELECT mule, ${BANK_QTY_SUM} AS qty
    FROM bank_transactions
    WHERE item_name = ?
    GROUP BY mule
    HAVING qty > 0
    ORDER BY mule COLLATE NOCASE
  `).all(itemName.trim());
}

/** Recent bank transactions, newest first. Optional exact item/mule filters. */
function getBankHistory({ item, mule, limit = 10 } = {}) {
  const conds  = [];
  const params = [];
  if (item) { conds.push('item_name = ?'); params.push(item.trim()); }
  if (mule) { conds.push('mule = ?');      params.push(mule); }
  const where = conds.length ? `WHERE ${conds.join(' AND ')}` : '';
  return getDb().prepare(`
...
/** Distinct mules with their current in-stock item counts and last activity. */
function getBankMules() {
  return getDb().prepare(`
    SELECT mule,
           SUM(CASE WHEN qty > 0 THEN 1 ELSE 0 END) AS item_count,
           MAX(last_activity) AS last_activity
    FROM (
      SELECT mule, item_name, ${BANK_QTY_SUM} AS qty, MAX(recorded_at) AS last_activity
      FROM bank_transactions
      GROUP BY mule, item_name
    )
    GROUP BY mule
    ORDER BY mule COLLATE NOCASE
  `).all();
...
/** Set (or replace) the persistent note on an (item, mule) inventory row. */
function setBankItemNote(itemName, mule, note, updatedBy) {
  getDb().prepare(`
    INSERT INTO bank_item_notes (item_name, mule, note, updated_by, updated_at)
    VALUES (?, ?, ?, ?, ?)
    ON CONFLICT(item_name, mule) DO UPDATE SET
      note       = excluded.note,
      updated_by = excluded.updated_by,
      updated_at = excluded.updated_at
  `).run(itemName.trim(), properCase(mule), note, updatedBy || null, Date.now());
}

/** Remove the persistent note on an (item, mule) inventory row. */
function clearBankItemNote(itemName, mule) {
  getDb().prepare(
    'DELETE FROM bank_item_notes WHERE item_name = ? AND mule = ?'
  ).run(itemName.trim(), mule);
}

/** Single bank transaction row, or null. */
function getBankTransaction(id) {
  return getDb().prepare('SELECT * FROM bank_transactions WHERE id = ?').get(id) ?? null;
}

/** Delete a bank transaction by id. Returns true if a row was removed. */
function deleteBankTransaction(id) {
  return getDb().prepare('DELETE FROM bank_transactions WHERE id = ?').run(id).changes > 0;
}

/** Distinct in-stock item names matching a partial term, for autocomplete. */
function searchBankItems(partial, limit = 25) {
  const conds  = [];
  const params = [];
  if (partial) { conds.push("item_name LIKE ? ESCAPE '\\'"); params.push(`%${partial.replace(/[%_\\]/g, '\\$&')}%`); }
  const where = conds.length ? `WHERE ${conds.join(' AND ')}` : '';
  return getDb().prepare(`
    SELECT item_name FROM bank_transactions
    ${where}
    GROUP BY item_name
    HAVING ${BANK_QTY_SUM} > 0
    ORDER BY item_name COLLATE NOCASE
    LIMIT ?
  `).all(...params, limit).map(r => r.item_name);
}
...
module.exports = {
  ...
  searchBankItems,
  /** Close the database cleanly (checkpoints WAL). */
  closeDb() { if (_db) { _db.close(); _db = null; } },
};
