...
const { saveRaid, mergeIntoRaid, getRaid, getRaidByDate, getCharsForDiscordId, linkCharacter, getDiscordInfoForChar, getAllAliases } = require('./db');
...
const MAX_BODY_BYTES = 5 * 1024 * 1024; // 5 MB — far above any real raid payload
...
function start(client) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // POST /raid/merge?id=<raidId> — merge into existing (checked before /raid)
    if (req.method === 'POST' && pathname === '/raid/merge') {
      try {
        const raidId = parseInt(parsedUrl.searchParams.get('id'), 10);
    ...
    if (req.method === 'GET' && pathname === '/raid') {
    ...
    // POST /raid — new raid
    if (req.method === 'POST' && pathname === '/raid') {
      try {
    ...
    // GET /guild-members — list all (non-bot) guild members for linking
    // Requires the GuildMembers privileged intent (Server Members Intent).
    if (req.method === 'GET' && pathname === '/guild-members') {
      ...
      try {
        ...
        const members = fetched
          ...
          .map(m => ({
            ...
            characters:  getCharsForDiscordId(m.id),
          }))
          .sort((a, b) => a.displayName.localeCompare(b.displayName));
        return send(res, 200, { members });
      } catch (err) {
        console.error('[API] /guild-members error:', err.message);
        return send(res, 500, { error: err.message });
      }
    }
    ...
    if (req.method === 'POST' && pathname === '/link-character') {
      try {
    ...
    if (req.method === 'GET' && pathname === '/character-info') {
    ...
    if (req.method === 'GET' && pathname === '/all-linked') {
