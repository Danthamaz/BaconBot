...
async function onVoiceStateUpdate(oldState, newState) {
  ...
  if (!wasInRaid && isInRaid) {
    // member can be null if not cached — fetch explicitly as fallback
    const member = newState.member
      ?? await newState.guild.members.fetch(newState.id).catch(() => null);
    if (member) await handleJoin(member);
    else console.warn(`[EventTracker] Could not resolve member for voice join: ${newState.id}`);
...
async function snapshotVoiceChannel(guild) {
  if (!activeRaid || !raidVoiceChannelId) return;
  try {
    // guild.voiceStates.cache is populated by GuildVoiceStates intent on GUILD_CREATE
    // and kept up to date by voiceStateUpdate events — no GuildMembers intent needed.
    const inChannel = guild.voiceStates.cache.filter(vs => vs.channelId === raidVoiceChannelId);
    console.log(`[EventTracker] Snapshot: ${inChannel.size} member(s) in voice channel`);
    const now = new Date();
    for (const [userId, voiceState] of inChannel) {
      const member = voiceState.member ?? await guild.members.fetch(userId).catch(() => null);
      if (member) await handleJoin(member, now);
      else console.warn(`[EventTracker] Could not resolve member ${userId} during snapshot`);
    }
  } catch (err) {
