...
async function snapshotVoiceChannel(guild) {
  ...
  try {
    ...
    if (!channel) {
      console.warn('[EventTracker] snapshotVoiceChannel: channel not found');
      return;
    }
    // channel.members relies on voice state cache; fetch guild members first
    // to ensure the cache is populated
    await guild.members.fetch();
    const now = new Date();
    const members = channel.members;
    console.log(`[EventTracker] Snapshot: ${members.size} member(s) in voice channel`);
